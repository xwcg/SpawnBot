...
namespace SBLists
{
    public class Main : SBPlugin
    {
        ...
        bool LoadLists ()
        {
            ...
            lists_index = Host.PluginConfigManager.Load<string>( this.PluginName, "index_lists.cfg" );
            if ( lists_index == null )
            {
                lists_index = new List<string>();
                return false;
            }
            foreach ( string list in lists_index )
            {
                List<string> thisList = Host.PluginConfigManager.Load<string>( this.PluginName, String.Format( "list_{0}.cfg", list ) );
                if ( thisList != null )
                    lists.Add( list, thisList );
            }
        ...
        bool SaveLists ()
        {
            try
            {
                ...
                if ( Host.PluginConfigManager.Save( lists_index, this.PluginName, "index_lists.cfg" ) == false )
                    return false;

                foreach ( KeyValuePair<string, List<string>> entry in lists )
                {
                    if ( Host.PluginConfigManager.Save( entry.Value, this.PluginName, String.Format( "list_{0}.cfg", entry.Key ) ) == false )
                        return false;
                }
            ...
            {
                Host.PluginResponse( "xwcg", "Error while saving:" + ex.Message );
                return false;
            }
        }
